...
#TEST: 'ALGUNAS PALABRAS SON GRAMATICALMENTE ERRÓNEAS. Pero se tiene que seguir transcribiendo'
...
def point_up(text: str):
    ...
    for i, word in enumerate(words):
        if is_case_one(clean_word(word)): #No se renderiza como los demás porque es un caso trivial
            if FLAGS['case'] == 2:
                    result_text += render_case(words[FLAGS['pos_ini']:FLAGS['pos_fin']+1], FLAGS['count'])
                    reset_flags()
